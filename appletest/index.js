...
function create_btn(type) {
    let keys = Object.getOwnPropertyNames(product[type]);
    model.innerHTML = '';
    keys.forEach((item, index) => {
        ...
        label.setAttribute('onclick', 'init_Iphone(this.innerText)');
...
function init_Iphone(product_type) {
    ...
    Name.forEach((item, index) => {
        if (Name[index] == 'color' || Name[index] == undefined) {
            // console.log(color);
        } else {
            ...
            p_wrap.classList.add('border', 'border-secondary', 'rounded', 'p-4', 'd-flex', 'justify-content-between');
